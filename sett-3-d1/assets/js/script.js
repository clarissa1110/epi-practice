...
const changeTitle = function () {
    document.getElementsByTagName('h1')[0].innerText = 'Questo è il Nuovo titolo';
}
changeTitle();
...
const changePcontent = function () {
    ...
    for (let i = 0; i < pContent.length; i++) {
        pContent[i].innerText = 'Questo è il nuovo testo dei p figli di div';
    }
};
changePcontent();
...
const changeUrls = function () { 
    const google = document.querySelector('a');
     for (let i = 0; i < google.length; i++) {
        const footer = google[i].closest('footer');

        if (!footer) {
            google[i].href = "https://www.google.com"
        }
     }
};
changeUrls();
...
const addParagraph = function () {
    ...
    newPar.innerText = 'Questo è un nuovo paragrafo';
